...
router.post('/', authenticateToken, async (req, res) => {
  ...
  if (!isMember) {
    return res
      .status(403)
      .json({ error: 'No tienes permisos para crear tareas en este proyecto' })
  }
...
router.get('/', authenticateToken, async (req, res) => {
  ...
  if (!isMember) {
    return res
      .status(403)
      .json({ error: 'No tienes permisos para ver tareas en este proyecto' })
  }
...
router.patch('/:taskId', authenticateToken, async (req, res) => {
  ...
  if (!isMember) {
    return res
      .status(403)
      .json({ error: 'No tienes permisos para actualizar esta tarea' })
  }
